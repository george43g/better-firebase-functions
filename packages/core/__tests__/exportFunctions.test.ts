...
describe('funcNameFromRelPathDefault', () => {
  ...
  it('converts kebab-case filename to camelCase', () => {
    expect(bff.funcNameFromRelPathDefault('camel-case-func.func.ts')).toBe('camelCaseFunc');
  });

  it('includes directory as group prefix', () => {
    expect(bff.funcNameFromRelPathDefault(`folder${sep}new.func.ts`)).toBe('folder-new');
  });

  it('handles deeply nested paths', () => {
    expect(bff.funcNameFromRelPathDefault(`folder${sep}nestedFolder${sep}sample-func.func.ts`)).toBe(
      'folder-nestedFolder-sampleFunc',
    );
  });
});
...
describe('exportFunctions() integration test suite', () => {
  ...
  const tempFuncDir = resolve(tmpdir(), `bff-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  const randOutput = Math.floor(Math.random() * 100) + 1;

  beforeAll(() => {
    generateTestDir(tempFuncDir, testFiles, `module.exports = { default: ${randOutput} };`);
  });
  ...
  it('should work without explicit __dirname (derived from __filename)', () => {
    ...
    expect(result).toHaveProperty(filePathToPropertyPath(testFiles[1]), randOutput);
  });
  ...
  it('should export file paths in exportPathMode', () => {
    ...
    expect(output).toHaveProperty(filePathToPropertyPath(testFiles[4]), testFiles[4].replace(/\//g, sep));
  });
  ...
  it('should use custom extractTrigger function', () => {
    const result = exportTestFactory({
      extractTrigger: (mod: any) => mod?.default ? `wrapped-${mod.default}` : null,
    });
    expect(result).toHaveProperty(filePathToPropertyPath(testFiles[1]), `wrapped-${randOutput}`);
  });
...
describe('exportFunctionsAsync() async test suite', () => {
  ...
  const tempFuncDir = resolve(tmpdir(), `bff-async-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  const randOutput = Math.floor(Math.random() * 100) + 1;
